...
HRESULT TutorialScene_3::init()
{
	...
	_player->getPlayerClass()->setPixelMap(IMAGEMANAGER->findImage("TUTORIAL_3_MAP_PIXEL"));
	...
	object* _object1;
	_object1 = new object;
	_object1->init("TENT_C", { 600, WINSIZEY - 180 });
	_vObject.push_back(_object1);

	object* _object2;
	_object2 = new object;
	_object2->init("FENCE_B", { 900, WINSIZEY - 180 });
	_vObject.push_back(_object2);

	object* _object3;
	_object3 = new object;
	_object3->init("WOODEN_CART_C", { 1200, WINSIZEY - 180 });
...
void TutorialScene_3::release()
{
}

void TutorialScene_3::update()
{
	if (!_FadeOut && !_FadeIn)
	{
		_player->setPlayerPositionBefore(_player->getPlayerClass()->getPlayerPosition());
		_player->update();
		_player->camMove(IMAGEMANAGER->findImage("TUTORIAL_3_MAP"));
	}
	...
	if (_FadeOut)
	{
		...
		{
			_FadeOut = false;
			SCENEMANAGER->loadScene("Tutorial_4", _player);
		}
	}
	...
	minimapUpdate(IMAGEMANAGER->findImage("TUTORIAL_3_MAP"), IMAGEMANAGER->findImage("MINI_TUTORIAL_3"));
}

void TutorialScene_3::render()
{
	...
	IMAGEMANAGER->findImage("TUTORIAL_3_MAP")->render(getMemDC(), _player->getPlayerClass()->getImagePosition().x, _player->getPlayerClass()->getImagePosition().y - (IMAGEMANAGER->findImage("TUTORIAL_3_MAP")->getHeight() - WINSIZEY));
	...
	minimapRender(IMAGEMANAGER->findImage("MINI_TUTORIAL_3"));
}

void TutorialScene_3::healHp()
{
	RECT temp;
	for (int i = 0; i < _vPotion.size(); i++)
	{
...
void TutorialScene_3::makePotion(object * _object)
{
...
void TutorialScene_3::minimapUpdate(image* originalField, image* minimapField)
{
	...
	minimapPlayerRc = RectMake(-6 +
		...
		minimapRc.top + (minimapField->getHeight()*(-(_player->getPlayerClass()->getImagePosition().y) + _player->getPlayerClass()->getPlayerPosition().y)) / originalField->getHeight() - 8, 8, 15);
	...
	minimapGateRc = RectMake(
		...
		((originalField->getHeight() - WINSIZEY) * ((float)minimapField->getHeight() / (float)originalField->getHeight())) +
		minimapRc.top + (minimapField->getHeight()*(-(_player->getPlayerClass()->getImagePosition().y) + _gate->getGateRect().bottom)) / originalField->getHeight() - 18, 15, 20);
}

void TutorialScene_3::minimapRender(image* minimapField)
{
